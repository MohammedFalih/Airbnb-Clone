            ...
            if (properties && properties.length != 0) {
                ...
                map.addListener("click", (mapMouseEvent) => {
                    if (setLatitude && setLongitude) {
                        setLatitude.value = mapMouseEvent.latLng.toJSON().lat;
                        setLongitude.value = mapMouseEvent.latLng.toJSON().lng;
                        ...
                        const marker = newAdvancedMarkerElement({
                            map: map,
                            position: mapMouseEvent.latLng.toJSON(),
                            content: location,
                        });
                        markers.push(marker)
                    }
